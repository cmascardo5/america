...
import com.america.config.EmbeddedKafka;
import com.america.config.KafkaSseConsumer;
import com.america.config.KafkaSseProducer;
...
class AmericaKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/america-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/america-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/america-kafka/unregister"));
                return;
            }
        }
